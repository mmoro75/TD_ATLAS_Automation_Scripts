...
def main():
    ...
    password = input("Enter hostname Password: ")
    lh_name = input("Enter Line Handler name : ")
    Atlas = input("Enter Venue tyep ATLAS od TD: ")
    ...
    if Atlas == "ATLAS":
        ...
        ric_list = input("Enter RIC name if multiple entries devided by space: ")
        ric_list = ric_list.split()
        choice = ("Enter Perist tyep DAT or LOADED: ")
        if choice == "DAT":
...
def upload_and_run_script(hostname, password, lh_name):
    try:
        ...
        return True
            ...
            socket.gaierror, TimeoutError, FileNotFoundError, ConnectionError, ConnectionRefusedError,
...
def download_persist(hostname, password, config_path, persist_file):
    try:
       ...
       # os.chmod(local_file_path, 0o666)
        ...
        sftp_client.get(remote_file_path, local_file_path + persist_file)
...
def check_persist(ric_list, persist_file):
    try:
        ...
        #    os.system(f"copy {filename} C:\\Users\\U6017127\.jenkins\\workspace\\TD_ATLAS_Persit_Analysis")
